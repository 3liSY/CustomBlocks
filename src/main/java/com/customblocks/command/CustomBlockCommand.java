...
public class CustomBlockCommand {
    ...
    public static void register() {
        CommandRegistrationCallback.EVENT.register((dispatcher, reg, env) -> {
            ...
            var tree = CommandManager.literal("customblock")
                ...
                // ── givetriangle <black|yellow|green> ────────────────────────────────────
    ...
    private static int cmdGiveTriangle(ServerCommandSource src, String color) {
        ...
        net.minecraft.util.Identifier trId =
            net.minecraft.util.Identifier.of(CustomBlocksMod.MOD_ID, normalized + "_triangle");
        net.minecraft.item.Item trItem = net.minecraft.registry.Registries.ITEM.get(trId);
        if (trItem == null || trItem == net.minecraft.item.Items.AIR) {
            src.sendError(Text.literal("§cTriangle item not found — is the mod loaded?")); return 0;
        }
        try {
            ServerPlayerEntity self = src.getPlayerOrThrow();
            self.getInventory().insertStack(new ItemStack(trItem, 1));
            String label = Character.toUpperCase(normalized.charAt(0)) + normalized.substring(1);
            src.sendMessage(Text.literal("§a[CustomBlocks] Given " + label + " Triangle. Right-click any Custom Block to recolour its background!"));
        } catch (Exception ex) {
